-- // The Survival Game Script, originally written for v3, ported to v2. Enjoy!
-- Now fixed!
...
local library = loadstring(game:HttpGet("https://raw.githubusercontent.com/Xcylinx/tsglua/main/library.lua", true), "ui")()
library:Init("tsg.lua v2.0")
...
local function waitForChild(parent, childName, timeOut)
    local key = parent:GetFullName() .. childName
...
local function resolvePath(parent, ...)
    local last = parent
    for i, v in next, {...} do 
        last = waitForChild(last, v)
    end

    return last
end

local worldResources = resolvePath(workspace, 'worldResources')
local placedStructures = resolvePath(workspace, 'placedStructures')
local mineable = resolvePath(worldResources, 'mineable')
local choppable = resolvePath(worldResources, 'choppable')
...
local tsg = {
    ...
    MeleeAnimalRemote = resolvePath(rs, "remoteInterface", "interactions", "meleeAI"),
...
-- Remove stupid on_lua_error function and get other stuff
for i, v in getgc() do
    if type(v) == 'function' then
        ...
        elseif name == 'humanoidStateChange' then
            print('Got humanoidStateChange function')
            tsg.humanoidStateChange = v
        end
    end
end

-- stack bypass
local stack = {'FiOne', `Players.{lplr.Name}.PlayerGui.client.client.tools.Tool.Collector`}
...
-- Basic idea is to get the function that fires the remotes legit and call it,
-- which calls the remotes (wally idea!) and then reverse/get the arg order from that
...
local FiOne; do
    ...
    FiOne.getupvalues = function(fn) 
        ...
        -- For some reason the upvalues table starts at 0? FiOne moment - possibly to replicate C?
        local upvals = debug.getupvalues(fn)
        local funcinfo = FiOne.getfuncinfo(fn)
        local upval_list = upvals[4]
        local num_upval = funcinfo.num_upval --#upval_list -- could also use funcinfo.num_upval?
...
local callRemote; do
    local ClientScript = lplr.PlayerGui:WaitForChild('client'):WaitForChild('client')
    ...
    -- So now we need to get the firing function name, which is as basic as getting them and checking if they are in the constants of the hitFunction
    -- Essentially checking what one is used and getting that, instead of getting fake ban remotes!

    local fireServerFunctionName
    local remote_tables = {}
    -- [DONE] Probably hook them and reverse which remote it is (instead of using order), while also reversing the argument order
    ...
    for i, v in hitFunctionsUpvalues do
        if type(v) ~= 'table' then continue end

        for i2, v2 in v do
            -- ignore stupid getmodel stuff
            if table.find(ignoreList, i2) then continue end
    ...
    local traceableArguments = {
        meleePlayer = {tsg.Items.ITEM_TYPES.MELEE_WEAPON, slotIdentifier,  {}, {hitObject = lplr, hitCF = cfIdentitier}},
        hitStructure = {tsg.Items.ITEM_TYPES.MELEE_WEAPON, slotIdentifier, {}, {hitObject = placedStructures:FindFirstChildWhichIsA('Model', true), hitCF = cfIdentitier}},
        chop = {tsg.Items.ITEM_TYPES.AXE, slotIdentifier,                  {}, {hitObject = choppable:FindFirstChildWhichIsA('Model', true), hitCF = cfIdentitier}},
        mine = {tsg.Items.ITEM_TYPES.PICKAXE, slotIdentifier,              {}, {hitObject = mineable:FindFirstChildWhichIsA('Model', true), hitCF = cfIdentitier }},
    }

    -- example: {meleePlayer = {"slot", "cframe", "instance"}}
    ...
    for i, v in remote_tables do 
        ...
        v[fireServerFunctionName] = function(self, ...)
            if (not remotes[currentRemote]) then
                print('[Script] Getting remote: ' .. currentRemote)
                ...
                print(`[Script] Got remote + arg order: [slot: {argumentOrder.slot}, instance: {argumentOrder.instance}{argumentOrder.cframe and `, cframe: {argumentOrder.cframe}]` or ']'}`)
    ...
    -- usage: callRemote('meleePlayer', {slot = slotHere, cframe = cfHere, instance = instanceHere})
    function callRemote(remote, params) 
...
print("[tsg.lua] Remote reversal took", os.clock() - start, "seconds")
...
local animalContainer = resolvePath(workspace, "AI_Server"); do 
                ...
                connection = runService.Heartbeat:Connect(function(dt) 
                    ...
                    if target then 
                        ...
                        if under.Enabled then 
                            ...
                            local goalPos = target.HumanoidRootPart.Position + Vector3.new(0, 7, 0)
...
if false then -- SERVER LAGGER [PATCHED]
...
if false then -- STaCK SIZE THING -- this is useless, as its now just in the normal game
    local oldMappings = {}
...
do
    ...
    local function setup() 
        ...
        for i, model in next, placedStructures:GetDescendants() do 
        ...
        table.insert(connections, placedStructures.DescendantAdded:Connect(function(model) 
            local hp = model:GetAttribute("health")
            if model:IsA("Model") and hp then 
                table.insert(tsg.Structures, model)
            end
        ...
        table.insert(connections, placedStructures.DescendantRemoving:Connect(function(model)
            local hp = model:GetAttribute("health")
            if model:IsA("Model") and hp then 
                    ...
                    repeat task.wait()
                        ...
                        for i, v in next, tsg.Structures do 
                            ...
                            if magCheck and healthCheck and localPlaceCheck and teamPlaceCheck then 
                                for i = 1, 3 do
                                    task.spawn(function()
                                        callRemote('hitStructure', {
                                            slot = bestPick or bestAxe or bestSword,
                                            instance = v,
...
do -- AUTO MINE
    ...
    local function updateMineables() 
        ...
        for i, descendant in next, worldResources:GetDescendants() do 
        ...
        table.insert(connections, worldResources.DescendantAdded:Connect(function(descendant) 
            if not descendant:IsA("Model") then 
                return
            end
        ...
        table.insert(connections, worldResources.DescendantRemoving:Connect(function(descendant) 
                    ...
                    repeat task.wait()
                        ...
                        for i, v in next,tsg.Mineables do 
                            ...
                            if magCheck and healthCheck then 
                                local remote = v.t == 'mineable' and 'mine' or 'chop'
    ...
    for i,v in next, worldResources:GetChildren() do 
...
do 
    ...
    highlight = library:Toggle({
        ...
        Function = function(value) 
            if value then 
                oldFunc = hookfunction(tsg.humanoidStateChange, function() end)
            else
                hookfunction(tsg.humanoidStateChange, oldFunc)
                oldFunc = nil
            end
        end,
        Tab = WorldTab,
